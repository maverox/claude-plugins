...
---
*Auto-generated by {skill} | Line {line_number} | Commit {short_sha}*
```

#### **Severity-Based Styling**

| Severity | Header Format | Status | Success |
|----------|---------------|--------|---------|
| Critical | `## 🔴 CRITICAL: {category}` | ❌ | ✅ |
| Important | `## 🟡 IMPORTANT: {category}` | ⚠️ | ✅ |
| Suggestion | `## 🟢 SUGGESTION: {category}` | 💡 | ✅ |

#### **Code Block Enhancement Rules**

1. **Always specify language** for syntax highlighting (rust, yaml, bash, json, etc.)
2. **Add inline annotations** in code blocks using comments
3. **Use consistent formatting** for current vs. suggested code

Example with annotations:
```rust
// ❌ AVOID: This pattern causes unnecessary allocation
let status = code.clone().into();

// ✅ PREFER: More efficient - consumes value directly
...
- **Failure Mode**: {specific_runtime_or_compile_failure}
- **Effort to Fix**: {Low|Medium|High}
...
#### **Complete Example - Critical Issue**
...
### Why This Matters
The UCS (Unified Commerce Stack) architecture requires `RouterDataV2` for proper connector integration. The legacy `RouterData` type lacks critical fields for payment routing and will cause compilation errors in the UCS environment. All new connectors must use V2 types.

### Impact Assessment
- **Severity**: Critical (P0 - Blocks Merge)
- **Failure Mode**: Compilation failure in UCS builds
- **Effort to Fix**: Low (simple import and type change)
- **Testing Required**: Full connector flow testing (authorize, capture, refund)

---
*Auto-generated by connector-code-quality-review | Line 25 | Commit abc123d*
```

#### **Complete Example - Important Issue**

```markdown
## 🟡 IMPORTANT: Status Mapping

**Issue**: Unknown refund status should map to `Pending`, not `Failure`

### Current Implementation
```rust
RefundStatus::from_str(status) match {
    "refunded" => Ok(Charged),
    "failed" => Ok(Failure),
    _ => Ok(Failure),  // ❌ Wrong: unknown → Failure
}
```
⚠️ Will incorrectly report unknown statuses as failed
...
RefundStatus::from_str(status) match {
    "refunded" => Ok(Charged),
    "failed" => Ok(Failure),
    _ => Ok(Pending),  // ✅ Correct: unknown → Pending
}
```
✅ Proper handling of unknown/in-progress statuses

### Why This Matters
Payment status mapping must distinguish between definitively failed payments (Failure) and unknown/processing states (Pending). Mapping unknown statuses to Failure can cause incorrect settlement reporting.

### Impact Assessment
- **Severity**: Important (P1 - Should Fix)
- **Code Quality Impact**: Correctness of refund reporting
- **Effort to Fix**: Low (one line change)
- **Testing Required**: Unit tests for status mapping edge cases

---
*Auto-generated by connector-integration-validator | Line 89 | Commit def456g*
```

#### **Complete Example - Suggestion**

```markdown
## 🟢 SUGGESTION: Code Organization

**Issue**: Consider extracting authentication logic into a separate function

### Current Implementation
```rust
fn create_payment(config: &Config) {
    // 30 lines of auth setup
...
### Suggested Fix
```rust
fn authenticate_request(config: &Config) -> Result<Token> {
    // 30 lines of auth setup
}

fn create_payment(config: &Config, token: Token) {
    // 20 lines of payment logic
    // 15 lines of error handling
}
```
...
### Why This Matters
Extracting authentication into a separate function improves testability, readability, and reusability. It makes the payment logic easier to understand at a glance.

### Impact Assessment
- **Severity**: Suggestion (P2 - Nice to Have)
- **Benefit**: Improved code readability and testability
- **Effort to Fix**: Low (simple refactoring)

---
*Auto-generated by connector-code-quality-review | Line 156 | Commit abc789h*
```
